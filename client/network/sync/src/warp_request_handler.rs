...
pub fn generate_request_response_config<Hash: AsRef<[u8]>>(
	...
	debug!(target: "warp-sync", "protocol name {}", generate_protocol_name(genesis_hash, fork_id));
	debug!(target: "warp-sync", "legacy protocol name {}", generate_protocol_name(genesis_hash, fork_id));
	RequestResponseConfig {
